...
def check_tupo_confirm(real_r,r):
    if (pos_obj['common-box-confirm'] in real_r and pos_obj['common-yellow-confirm'] in real_r) or \
        (pos_obj['common-yellow-confirm'] in real_r and len(r[pos_obj['common-yellow-confirm']]) == 2) or\
        (pos_obj['common-yellow-confirm'] in real_r and pos_obj['realm-ticket'] in real_r and  len(r[pos_obj['realm-ticket']]) == 2):
        return True
    return False    
...
def no_k28_check(image):
    ...
    for r in result:
        ...
        if yys_config.location != location_codes['attack']:
            if pos_obj['ready'] in real_r or pos_obj['attack-exit'] in real_r:
                return True,'attack page',r
            elif pos_obj['auto-logo'] in real_r or pos_obj['flame'] in real_r or pos_obj['willpower'] in real_r: # 自动战斗标志
                return True,"attacking",r
        elif pos_obj['common-red-cancel'] in real_r: # 红色取消
            return True,"attack_exit",r
        elif pos_obj['attack-exit'] in real_r or pos_obj['auto-logo'] in real_r or pos_obj['flame'] in real_r or pos_obj['willpower'] in real_r: # 自动战斗标志
            return True,"attacking",r
        
        if yys_config.location != location_codes['tupo']:
            if pos_obj['common-red-exit'] in real_r and (pos_obj['realm-success'] in real_r or pos_obj['realm-again'] in real_r or pos_obj['realm-wait'] in real_r): # 35: 已攻破；34: 未攻破
                return True,"in tupo choose page",r
        else:
            if pos_obj['success-damo'] in real_r:
                return True,'tupo success page',r
            elif pos_obj['common-red-cancel'] in real_r: # 红色取消
                return True,"tupo fresh",r
            elif check_tupo_confirm(real_r,r):
                return True, "in tupo confirm page",r
            elif pos_obj['common-red-exit'] in real_r and (pos_obj['realm-success'] in real_r or pos_obj['realm-again'] in real_r or pos_obj['realm-wait'] in real_r): # 35: 已攻破；34: 未攻破
                return True,"keep tupo choose page",r
        
        if yys_config.location != location_codes['k28']:
            if yys_config.step == 3 and (pos_obj['common-blue-exit'] in real_r and pos_obj['shiki-dir'] in real_r):
                return True,"in k28",r
            elif pos_obj['realm-logo'] not in real_r and pos_obj['common-red-exit'] not in real_r and (pos_obj['common-blue-exit'] in real_r and pos_obj['shiki-dir'] in real_r):
                return True,"in k28",r
        elif pos_obj['k28-success-box'] in real_r or (pos_obj['common-blue-exit'] in real_r and pos_obj['shiki-dir'] in real_r):
            return True,"keep in k28",r
        elif pos_obj['common-yellow-confirm'] in real_r and pos_obj['common-red-exit'] not in real_r and len(r[pos_obj['common-yellow-confirm']]) == 2:
            return True, "k28 exit page",r

        if yys_config.location != location_codes['settlement'] and yys_config.location != location_codes['k28']:
            if pos_obj['success-damo'] in real_r:
                return True,'maybe success page',r
            elif yys_config.step == 2 and pos_obj['again-attack'] in real_r and yys_config.location == location_codes['attack']:
                return True,"failed",r
            elif pos_obj['failed-logo'] in real_r and yys_config.location == location_codes['attack']:
                return True,"maybe end",r
        elif yys_config.step == 2 and pos_obj['again-attack'] in real_r:
            return True,"failed",r
        ...
        if yys_config.location != location_codes['tansuo']:
            if pos_obj['realm-logo'] in real_r and pos_obj['k28-box-small'] in real_r and pos_obj['buff-logo'] in real_r \
                and pos_obj['common-red-exit'] not in real_r:
                return True,"TanSuo Root",r
        elif (pos_obj['realm-logo'] in real_r or pos_obj['k28-box-small'] in real_r) and pos_obj['buff-logo'] in real_r:
            return True,"keep TanSuo Root",r

        if yys_config.location != location_codes['k28_box']:
            if pos_obj['k28-box-big'] in real_r:
                return True,"k28 enter",r
            elif pos_obj['common-yellow-confirm'] in real_r and pos_obj['buff-logo'] in real_r and pos_obj['common-red-exit'] in real_r:
                return True,"maybe k28 enter",r
        elif pos_obj['k28-box-big'] in real_r or (pos_obj['common-yellow-confirm'] in real_r and pos_obj['buff-logo'] in real_r and pos_obj['common-red-exit'] in real_r):
            return True,"keep k28 enter",r

    if yys_config.click_number % 5 == 0:
...
def k28():
    pos_num = 0 ; success_flag = 0 ; failed_flag = 0 ; while_flag = 0
    ...
    while True:
        if not result: # 未识别到突破或者k28页面
            ...
            if yys_config.location == location_codes['k28_box']:
                return
            elif yys_config.location == location_codes['tansuo']:
                yys_config.step = 0
                return
            elif yys_config.location == location_codes['k28']:
                yys_config.k28_state = state['choose']
...
def tupo_attack(xy):
    tupo_state = 0 ; success_flag =  0 ; failed_flag = 0 ; click_num = 0
        ...
        if not result: # 未识别到突破或者k28页面
            ...
            if yys_config.location == location_codes['settlement']:
                ...
                if pos_obj['common-red-cancel'] not in r.keys():
                    ...
                    enter(r,7)
            elif 'attack_exit' in message:
                enter(r,7)
            # elif yys_config.location != location_codes['attack']:
            # tupo_state = state['Settlement']
        elif tupo_state == state['Settlement']:
            if 'attack_exit' in message:
                enter(r,7)
...
def enter(r,location):
    ...
    if location == location_codes['tupo']: # 突破按钮
        ...
        if pos_obj['k28-box-small'] in r.keys():
            xy = [0,0,0,0]
